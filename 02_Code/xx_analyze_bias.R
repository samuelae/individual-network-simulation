  ...
  for(study in 1:nrow(study_parameters)) {
    ...
    for(cue_set in 1:10) {
      ...
      results <- list(parameters = list(indy_subj = this_behav_data$indy_subj,
                      ...
                      importance.bias.fa = bias(estimated = fa_imp_values, true = indy_imp_values, na.rm = TRUE),
                      importance.bias.rj = bias(estimated = rj_imp_values, true = indy_imp_values, na.rm = TRUE),
                      relatedness.bias.fa = bias(estimated = fa_rel_values, true = indy_rel_values, na.rm = TRUE),
                      relatedness.bias.rj = bias(estimated = rj_rel_values, true = indy_rel_values, na.rm = TRUE))
      
      results_multiset[[cue_set]] <- results
      
    } # end for cue_set
    
    saveRDS(results_multiset, paste0(analysis_results_folder, "bias_results_param_", 
                                     sprintf("%02d", indy_param),  "_subj_", 
                                     sprintf("%02d", indy_subj), "_study_", 
                                     sprintf("%02d", study), ".rds"))
...
for (subject in 1:10) {
  ...
  stopCluster(cl)
  gc()
  tictoc::toc() # x min (runs on sciCORE server with 28 cores)
  print(paste("subject", subject, "complete"))
}
